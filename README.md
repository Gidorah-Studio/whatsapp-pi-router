...
Quoted images use the existing bounded downloader and are attached separately (current image first, quoted image second when both exist). Quoted PDFs use the bounded document downloader, receive the existing PDF text extraction, and are saved only in the private per-turn directory so cleanup removes them after the reply. Unavailable images or PDFs request a resend rather than guessing from captions. This feature does not newly transcribe quoted audio or extract quoted video/non-PDF documents.
